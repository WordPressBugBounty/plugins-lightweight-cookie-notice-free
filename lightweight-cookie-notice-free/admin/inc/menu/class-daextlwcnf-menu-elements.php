 ...
 * Parent class used to create the admin pages.
 */
class Daextlwcnf_Menu_Elements {
	 ...
	 * The constructor.
	 *
	 * @param Daextlwcnf_Shared $shared An instance of the shared class.
	...
	public function display_list_table(
		...
		if ( ! is_null( $post_search_input ) && mb_strlen( trim( $post_search_input ) ) > 0 ) {
			...
			$total_items = $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->prefix}daextlwcnf_$db_table $filter" );

			// Initialize the pagination class.
			require_once $this->shared->get( 'dir' ) . '/admin/inc/class-daextlwcnf-pagination.php';
			$pag = new Daextlwcnf_Pagination( $this->shared );
		...
		?>
			...
			$results = $wpdb->get_results(
				"SELECT * FROM {$wpdb->prefix}daextlwcnf_$db_table $filter ORDER BY $db_primary_key DESC $query_limit",
				ARRAY_A
			);
			// phpcs:enable
			...
			<?php if ( count( $results ) > 0 ) : ?>
				...
				<div class="daextlwcnf-crud-table">

					<!-- list of tables -->
					<table class="daextlwcnf-crud-table__daext-items">
						<thead>
						<tr>
							<th>
								<input type="checkbox" class="daextlwcnf-cb-select-all">
							</th>
							<?php
						...
						<?php foreach ( $results as $result ) : ?>
							<tr>
								<td><input type="checkbox"
											class="daextlwcnf-bulk-action-checkbox"
								...
								foreach ( $columns as $key => $column ) {
									...
									<td>
										...
										<?php if ( 0 === $key ) : ?>
											<a class="daextlwcnf-crud-table__item-name" href="admin.php?page=<?php echo esc_attr( $this->shared->get( 'slug' ) ); ?>-<?php echo esc_attr( $page_slug ); ?>&edit_id=<?php echo esc_attr( $result[ $db_primary_key ] ); ?>">
												<?php echo esc_html( stripslashes( $result[ $column['db_field'] ] ) ); ?>
											</a>
											<div class="daextlwcnf-crud-table__row-actions">
											<div class="daextlwcnf-crud-table__row-actions-single-action">
												<a href="admin.php?page=<?php echo esc_attr( $this->shared->get( 'slug' ) ); ?>-<?php echo esc_attr( $page_slug ); ?>&edit_id=<?php echo esc_attr( $result[ $db_primary_key ] ); ?>"><?php esc_html_e( 'Edit', 'lightweight-cookie-notice-free' ); ?></a>
											</div>
											<div>&nbsp|&nbsp</div>
											<div class="daextlwcnf-crud-table__row-actions-single-action">
												<form method="POST"
													...
													<?php wp_nonce_field( 'daextlwcnf_clone_' . $this->menu_slug . '_' . intval( $result[ $db_primary_key ], 10 ), 'daextlwcnf_clone_' . $this->menu_slug . '_nonce' ); ?>
													<button type="submit" name="clone_id" value="<?php echo esc_html( $result[ $db_primary_key ] ); ?>"><?php esc_html_e( 'Duplicate', 'lightweight-cookie-notice-free' ); ?></button>
												</form>
											</div>
											<div>&nbsp|&nbsp</div>
											<div class="daextlwcnf-crud-table__row-actions-single-action daextlwcnf-crud-table__row-actions-single-action-delete">
												<form method="POST"
														action="admin.php?page=<?php echo esc_attr( $this->shared->get( 'slug' ) ); ?>-<?php echo esc_attr( $page_slug ); ?>"
														id="delete-item-<?php echo esc_attr( $result[ $db_primary_key ] ); ?>">
													<?php wp_nonce_field( 'daextlwcnf_delete_' . $this->menu_slug . '_' . intval( $result[ $db_primary_key ], 10 ), 'daextlwcnf_clone_' . $this->menu_slug . '_nonce' ); ?>
													<input type="hidden" name="delete_id" value="<?php echo esc_html( $result[ $db_primary_key ] ); ?>">
													<button type="submit" value="<?php echo esc_html( $result[ $db_primary_key ] ); ?>"><?php esc_html_e( 'Delete', 'lightweight-cookie-notice-free' ); ?></button>
												</form>
											</div>
										</div>
				...
				<!-- Bulk Actions -->
				<div class="daextlwcnf-crud-table-controls">

					<div class="daextlwcnf-crud-table-controls__bulk-actions">
						<form method="POST" action="admin.php?page=<?php echo esc_attr( $this->shared->get( 'slug' ) ); ?>-<?php echo esc_attr( $this->slug_plural ); ?>">
							<select name="bulk_action" id="bulk_action">
								<option value=""><?php esc_html_e( 'Bulk actions', 'lightweight-cookie-notice-free' ); ?></option>
								<option value="delete"><?php esc_html_e( 'Delete', 'lightweight-cookie-notice-free' ); ?></option>
							</select>
							<?php wp_nonce_field( 'daextlwcnf_bulk_action_' . $this->menu_slug, 'daextlwcnf_bulk_action_' . $this->menu_slug . '_nonce' ); ?>
							<input id="bulk-action-selected-items" type="hidden" name="bulk-action-selected-items" value="">
							<input id="daextlwcnf-submit-bulk-action" type="submit" class="button daextlwcnf-admin-page-button" value="<?php esc_html_e( 'Apply', 'lightweight-cookie-notice-free' ); ?>">
						</form>
					</div>

					<div class="daextlwcnf-crud-table-controls__pagination-container">
						<!-- Display the pagination -->
						<?php if ( $pag->total_items > 0 ) : ?>
							<div class="daextlwcnf-crud-table-controls__daext-tablenav">
									<span class="daextlwcnf-crud-table-controls__daext-displaying-num"><?php echo esc_html( $pag->total_items ); ?>&nbsp<?php esc_html_e( 'items', 'lightweight-cookie-notice-free' ); ?></span>
									<?php $pag->show(); ?>
							</div>
						<?php endif; ?>
					</div>
			...
			<?php else : ?>
				...
				if ( mb_strlen( trim( $filter ) ) > 0 ) {
					$this->shared->save_dismissible_notice(
						__( 'There are no results that match your filter.', 'lightweight-cookie-notice-free' ),
						'updated'
					);
				}
				...
				if ( 0 === mb_strlen( trim( $filter ) ) ) {
					echo '<div class="daextlwcnf-crud-table__no-items-found-message">' . esc_html__( 'Nothing to show yet! Add some items by clicking the Add New button.', 'lightweight-cookie-notice-free' ) . '</div>';
				}
	...
	public function header_bar() {
		...
		if ( 'new' === $action ) {
			$page_title = __( 'Add New', 'lightweight-cookie-notice-free' ) . ' ' . $this->label_singular;
		} elseif ( null !== $edit_id ) {
			$page_title = __( 'Edit', 'lightweight-cookie-notice-free' ) . ' ' . $this->label_singular;
		} else {
		...
		<div class="daextlwcnf-header-bar">

			<div class="daextlwcnf-header-bar__left">
				<div class="daextlwcnf-header-bar__page-title"><?php echo esc_html( $page_title ); ?></div>
				<?php if ( 'list' === $action && 'crud' === $this->context && null === $edit_id ) : ?>
					<a href="<?php echo esc_url( get_admin_url() . 'admin.php?page=daextlwcnf-' . $this->slug_plural . '&action=new' ); ?>"
						class="daextlwcnf-button daextlwcnf-header-bar__add-new-button">
						<?php $this->shared->echo_icon_svg( 'plus' ); ?>
						<div class="daextlwcnf-header-bar__add-new-button-text"><?php esc_html_e( 'Add New', 'lightweight-cookie-notice-free' ); ?></div>
					</a>
				<?php endif; ?>
			</div>

			<div class="daextlwcnf-header-bar__right">
				<?php if ( 'new' === $action || null !== $edit_id ) : ?>
					<a href="#" onclick="document.getElementById('form1').submit()" class="daextlwcnf-btn daextlwcnf-btn-primary"><?php esc_html_e( 'Save Changes', 'lightweight-cookie-notice-free' ); ?></a>
				<?php endif; ?>
			</div>
	...
	public function new_item() {
		...
		<div class="daextlwcnf-main-form">
			...
			<form id="form1" name="form1" method="POST"
					action="admin.php?page=daextlwcnf-<?php echo esc_attr( $this->slug_plural ); ?>"
					autocomplete="off">
				...
				wp_nonce_field( 'daextlwcnf_create_update_' . $this->menu_slug, 'daextlwcnf_create_update_' . $this->menu_slug . '_nonce' );
	...
	public function edit_item( $edit_id ) {
		...
		<div class="daextlwcnf-main-form">
			...
			<form id="form1" name="form1" method="POST"
				...
				// Sanitize the field name.
				$db_table_name = $wpdb->prefix . 'daextlwcnf_' . $this->db_table;
	...
	public function section_header( $label, $section_id, $icon_id = null ) {
		...
		<div class="daextlwcnf-main-form__section-header group-trigger" data-trigger-target="<?php echo esc_attr( $section_id ); ?>">
			<div class="daextlwcnf-main-form__section-header-title">
				<?php $this->shared->echo_icon_svg( $icon_id ); ?>
				<div class="daextlwcnf-main-form__section-header-title-text"><?php echo esc_html( $label ); ?></div>
			</div>
			<div class="daextlwcnf-main-form__section-header-toggle">
	...
	public function input_field(
		...
		<div class="daextlwcnf-main-form__daext-form-field" valign="top" data-section-id="<?php echo esc_attr( $section_id ); ?>">
			<div><label for="title"><?php echo esc_html( $label ); ?><?php echo $required ? ' <span class="daextlwcnf-required">*</span>' : ''; ?></label></div>
			<div>
	...
	public function display_admin_toolbar() {
		...
		<div class="daextlwcnf-admin-toolbar">
			<div class="daextlwcnf-admin-toolbar__left-section">
				<div class="daextlwcnf-admin-toolbar__menu-items">
					<a href="<?php echo esc_url( admin_url( 'admin.php?page=daextlwcnf-consent-log' ) ); ?>" class="daextlwcnf-admin-toolbar__plugin-logo">
						<img src="<?php echo esc_url( $this->shared->get( 'url' ) . 'admin/assets/img/plugin-logo.svg' ); ?>" alt="Lightweight Cookie Notice" />
					</a>
					<?php
					...
					foreach ( $this->config['admin_toolbar']['items'] as $key => $item ) {
						...
						<a href="<?php echo esc_attr( $item['link_url'] ); ?>" class="daextlwcnf-admin-toolbar__menu-item <?php echo 'daextlwcnf-' . $this->menu_slug === $item['menu_slug'] ? 'is-active' : ''; ?>">
							<div class="daextlwcnf-admin-toolbar__menu-item-wrapper">
								<?php $this->shared->echo_icon_svg( $item['icon'] ); ?>
								<div class="daextlwcnf-admin-toolbar__menu-item-text"><?php echo esc_html( $item['link_text'] ); ?></div>
							</div>
						</a>
					...
					<div class="daextlwcnf-admin-toolbar__menu-item daextlwcnf-admin-toolbar__menu-item-more">
						<div class="daextlwcnf-admin-toolbar__menu-item-wrapper">
							<?php $this->shared->echo_icon_svg( 'grid-01' ); ?>
							<div class="daextlwcnf-admin-toolbar__menu-item-text"><?php esc_html_e( 'More', 'lightweight-cookie-notice-free' ); ?></div>
							<?php $this->shared->echo_icon_svg( 'chevron-down' ); ?>
						</div>
						<ul class="daextlwcnf-admin-toolbar__pop-sub-menu">
							...
							foreach ( $this->config['admin_toolbar']['more_items'] as $key => $more_item ) {
								...
								<li>
									<a href="<?php echo esc_attr( $more_item['link_url'] ); ?>" <?php echo 1 === intval( $more_item['pro_badge'], 10 ) ? 'target="_blank"' : ''; ?>>
										<?php echo '<div class="daextlwcnf-admin-toolbar__more-item-item-text">' . esc_html( $more_item['link_text'] ) . '</div>'; ?>
										<?php

										if ( true === isset( $more_item['pro_badge'] ) && $more_item['pro_badge'] ) {
											echo '<div class="daextlwcnf-admin-toolbar__pro-badge">' . esc_html__( 'PRO', 'lightweight-cookie-notice-free' ) . '</div>';
										}
			...
			<div class="daextlwcnf-admin-toolbar__right-section">
				<!-- Display the upgrade button in the Free version. -->
				<?php if ( constant( 'DAEXTLWCN_EDITION' ) === 'FREE' ) : ?>
				<a href="https://daext.com/lightweight-cookie-notice/" target="_blank" class="daextlwcnf-admin-toolbar__upgrade-button">
					<?php $this->shared->echo_icon_svg( 'diamond-01' ); ?>
					<div class="daextlwcnf-admin-toolbar__upgrade-button-text"><?php esc_html_e( 'Unlock Extra Features with LCN Pro', 'lightweight-cookie-notice-free' ); ?></div>
				</a>
				<?php endif; ?>
				<a href="https://daext.com" target="_blank" class="daextlwcnf-admin-toolbar__daext-logo-container">
				<img class="daextlwcnf-admin-toolbar__daext-logo" src="<?php echo esc_url( $this->shared->get( 'url' ) . 'admin/assets/img/daext-logo.svg' ); ?>" alt="DAEXT" />
				</a>
			</div>
		</div>
	...
	public function display_pro_features() {
		...
		if ( constant( 'DAEXTLWCN_EDITION' ) !== 'FREE' ) {
			return;
		}
		...
		<div class="daextlwcnf-admin-body">
			...
			<div class="daextlwcnf-pro-features">

				<div class="daextlwcnf-pro-features__wrapper">

					<div class="daextlwcnf-pro-features__left">
						<div class="daextlwcnf-pro-features__title">
							<div class="daextlwcnf-pro-features__title-text"><?php esc_html_e( 'Unlock Advanced Features with Lightweight Cookie Notice Pro', 'lightweight-cookie-notice-free' ); ?></div>
							<div class="daextlwcnf-pro-features__pro-badge"><?php esc_html_e( 'PRO', 'lightweight-cookie-notice-free' ); ?></div>
						</div>
						<div class="daextlwcnf-pro-features__description">
							<?php
							esc_html_e(
								'Download the proof of consent in PDF format, export consent log data in CSV format, set up Google consent mode, configure subdomains, add lists of cookies and cookie reset buttons to standard pages, and more!',
								'lightweight-cookie-notice-free'
							);
							?>
						</div>
						<div class="daextlwcnf-pro-features__buttons-container">
							<a class="daextlwcnf-pro-features__button-1" href="https://daext.com/lightweight-cookie-notice/" target="_blank">
								<div class="daextlwcnf-pro-features__button-text">
									<?php esc_html_e( 'Learn More', 'lightweight-cookie-notice-free' ); ?>
								</div>
								<?php $this->shared->echo_icon_svg( 'arrow-up-right' ); ?>
							</a>
							<a class="daextlwcnf-pro-features__button-2" href="https://daext.com/lightweight-cookie-notice/#pricing" target="_blank">
								<div class="daextlwcnf-pro-features__button-text">
									<?php esc_html_e( 'View Pricing & Upgrade', 'lightweight-cookie-notice-free' ); ?>
								</div>
								<?php
					...
					<div class="daextlwcnf-pro-features__right">
						...
						$pro_features_data_a = array(
							...
							array(
								'icon'        => 'book-open-01',
								'name_part_1' => 'Cookie',
								'name_part_2' => 'Catalogs',
							),
							array(
								'icon'        => 'target-03',
								'name_part_1' => 'Geolocation',
								'name_part_2' => 'Detection',
							),
							array(
								'icon'        => 'share-05',
						...
						foreach ( $pro_features_data_a as $key => $pro_feature_data ) {
							...
							<div class="daextlwcnf-pro-features__single-feature">
								<div class="daextlwcnf-pro-features__single-feature-wrapper">
									<?php $this->shared->echo_icon_svg( $pro_feature_data['icon'] ); ?>
									<div class="daextlwcnf-pro-features__single-feature-name">
				...
				<div class="daextlwcnf-pro-features__footer-wrapper">
					<div class="daextlwcnf-pro-features__footer-wrapper-inner">
						<div class="daextlwcnf-pro-features__footer-wrapper-left">
							<?php esc_html_e( 'Built for WordPress creators by the DAEXT team', 'lightweight-cookie-notice-free' ); ?>
						</div>
						<a class="daextlwcnf-pro-features__footer-wrapper-right" href="https://daext.com/products/" target="_blank">
							<div class="daextlwcnf-pro-features__footer-wrapper-right-text">
								<?php esc_html_e( 'More Tools from DAEXT', 'lightweight-cookie-notice-free' ); ?>
							</div>
							<?php $this->shared->echo_icon_svg( 'arrow-up-right' ); ?>
						</a>
					</div>
	...
	public function handle_duplicate() {
		...
		if ( ! is_null( $data['clone_id'] ) ) {
			...
			// Nonce verification.
			check_admin_referer( 'daextlwcnf_clone_' . $this->menu_slug . '_' . $data['clone_id'], 'daextlwcnf_clone_' . $this->menu_slug . '_nonce' );

			$this->duplicate_record( $this->db_table, $this->primary_key, $data['clone_id'] );

			$this->shared->save_dismissible_notice(
				__( 'The item has been successfully duplicated.', 'lightweight-cookie-notice-free' ),
				'updated'
			);
	...
	public function handle_delete() {
		...
		if ( ! is_null( $data['delete_id'] ) ) {
			...
			// Nonce verification.
			check_admin_referer( 'daextlwcnf_delete_' . $this->menu_slug . '_' . $data['delete_id'], 'daextlwcnf_clone_' . $this->menu_slug . '_nonce' );
			...
			if ( ! $result['is_deletable'] ) {
				...
				if ( false !== $query_result ) {
					$this->shared->save_dismissible_notice(
						__( 'The item has been successfully deleted.', 'lightweight-cookie-notice-free' ),
						'updated'
					);
				}
			}
	...
	public function handle_bulk_actions() {
		...
		if ( 'delete' === $bulk_action ) {
			...
			if ( ! is_null( $delete_id ) ) {
				...
				// Nonce verification.
				check_admin_referer( 'daextlwcnf_bulk_action_' . $this->menu_slug, 'daextlwcnf_bulk_action_' . $this->menu_slug . '_nonce' );
				...
				if ( isset( $query_result ) && false !== $query_result ) {
					...
					$this->shared->save_dismissible_notice(
						$deleted_items_count . ' ' . __( 'items have been successfully deleted.', 'lightweight-cookie-notice-free' ),
						'updated'
					);
				...
				if ( count( $delete_id_non_deletable ) > 0 ) {
					...
					switch ( $this->menu_slug ) {
						...
						case 'section':
							$this->shared->save_dismissible_notice(
								__( 'The', 'lightweight-cookie-notice-free' ) . ' ' . strtolower( $this->label_plural ) . ' ' . __( "with the following IDs are used in one or more categories and can't be deleted:", 'lightweight-cookie-notice-free' ) . ' ' . implode( ', ', $delete_id_non_deletable ) . '.',
								'error'
							);
						...
						case 'category':
							$this->shared->save_dismissible_notice(
								__( 'The', 'lightweight-cookie-notice-free' ) . ' ' . strtolower( $this->label_plural ) . ' ' . __( "with the following IDs are used in one or more cookies and can't be deleted:", 'lightweight-cookie-notice-free' ) . ' ' . implode( ', ', $delete_id_non_deletable ) . '.',
								'error'
							);
	...
	public function print_form_fields_from_array( $sections ) {
		...
		foreach ( $sections as $section ) {
			...
			echo '<div class="daextlwcnf-main-form__daext-form-section" data-id="' . esc_attr( $section['section_id'] ) . '">';
			...
			echo '<div class="daextlwcnf-main-form__daext-form-section-body" data-section-id="' . esc_attr( $section['section_id'] ) . '">';
	...
	public function display_crud_menu_search_form( $post_search_input ) {
		...
		<form action="admin.php" method="GET" id="items-filter">
			...
			<div class="daextlwcnf-crud-table-search-form">

				<div id="daext-search-form">

					<input type="hidden" name="page" value="daextlwcnf-<?php echo esc_attr( $this->slug_plural ); ?>">
					<input class="daextlwcnf-crud-table-search-form__post-search-input" type="text" name="s"
							value="<?php echo null !== $post_search_input ? esc_attr( stripslashes( $post_search_input ) ) : ''; ?>" autocomplete="off" maxlength="255">
					<input class="button daextlwcnf-admin-page-button" type="submit" value="Search <?php echo esc_attr( $this->label_plural ); ?>">
